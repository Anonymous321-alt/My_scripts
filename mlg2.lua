...
whBtn.Text=WH_ENABLED and"WALLHOP: ON"or"WALLHOP: OFF"
whBtn.BackgroundColor3=WH_ENABLED and Color3.fromRGB(40,160,90)or Color3.fromRGB(150,50,50)
if not WH_ENABLED then
WH_hops=0
WH_stopTwist()
if WH_curAnimTrack then
...
SV(Rg)
-- HUD — попадёт ли удар
local HUD=Instance.new("ScreenGui",L:WaitForChild("PlayerGui"))
HUD.Name="PunchHUD"
HUD.ResetOnSpawn=false
local box=Instance.new("Frame",HUD)
box.Size=UDim2.new(0,200,0,44)
box.Position=UDim2.new(1,-220,0,20)
...
-- ищем ближайшего игрока от МОЕГО Hitbox (не от HRP)
local function findNearestByHitbox()
local c=L.Character
if not c then return nil end
local hb=c:FindFirstChild("Hitbox")
...
local q=p.Character:FindFirstChild("HumanoidRootPart")
local u=p.Character:FindFirstChild("Humanoid")
if q and u and u.Health>0 then
local arm=p.Character:FindFirstChild("Right Arm")or p.Character:FindFirstChild("RightHand")or q
local d=(arm.Position-hb.Position).Magnitude
if d<bd then best,bd={char=p.Character,name=p.Name},d end
end
end
end
if best then return{char=best.char,name=best.name,dist=bd}end
return nil
end
...
if not hb then
lbl.Text="НЕТ ХИТБОКСА"
lbl.TextColor3=Color3.fromRGB(150,150,150)
box.BackgroundColor3=Color3.fromRGB(20,20,28)
else
local t=findNearestByHitbox()
if t then
local d=t.dist
if d<=12 then
lbl.Text="ПОПАДЁТ ("..string.format("%.0f",d)..")"
lbl.TextColor3=Color3.fromRGB(0,255,80)
box.BackgroundColor3=Color3.fromRGB(0,60,20)
elseif d<=18 then
lbl.Text="ПОЧТИ ("..string.format("%.0f",d)..")"
lbl.TextColor3=Color3.fromRGB(255,220,0)
box.BackgroundColor3=Color3.fromRGB(60,50,0)
else
lbl.Text="НЕ ПОПАДЁТ ("..string.format("%.0f",d)..")"
lbl.TextColor3=Color3.fromRGB(255,100,100)
box.BackgroundColor3=Color3.fromRGB(60,20,20)
end
else
lbl.Text="НЕТ ЦЕЛИ РЯДОМ"
